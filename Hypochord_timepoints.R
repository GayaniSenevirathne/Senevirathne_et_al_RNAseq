...
resultsNames(dds) 
res <- results(dds, contrast=c("timepoints","t1","t2"))
resultsNames(dds) #this gives the contrasts that can be done
res
summary(res)
res1 <- results(dds, contrast = c("timepoints","t1","t2"))
res1 <- lfcShrink(dds, contrast = c("timepoints","t1","t2"), res = res1)
...
res05 <- results(dds, contrast=c("timepoints","t1","t2"), alpha=0.05)
summary(res05)
...
pdf(file="p_0.0005_hypochord_t1vst2_geneIDs_50_genes.pdf")
...
pdf(file="Hypo_vs_Meth_volcanoplot_Selected_genes_t1vst2_labelled2.pdf")  
